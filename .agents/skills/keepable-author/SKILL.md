...
Use the one-pass scanner below instead of `grep` globbing. It reads only frontmatter, handles both inline and list-style `author` fields, mirrors the folders in `3 - Bases/All Links.base`, and compares exact author names to `Author/*.md` filenames.
